...
namespace RawInput_dll
{
	public sealed class RawKeyboard
	{
		...
		public void EnumerateDevices()
		{
			lock (_padLock)
			{
				...
				if (Win32.GetRawInputDeviceList(IntPtr.Zero, ref deviceCount, (uint)dwSize) == 0)
				{
					...
					Debug.WriteLine(string.Format( "EnumerateDevices() found {0} Keyboard(s)", NumberOfKeyboards));
					return;
				}
			}
        ...
        public void ProcessRawInput(IntPtr hdevice)
		{
			...
			{
				Debug.WriteLine(string.Format("Handle: {0} was not in the device list.", _rawBuffer.header.hDevice));
				return;
			}
